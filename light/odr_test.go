...
var (
	...
	testBankFunds   = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
...
type testOdr struct {
	...
	sdb, ldb      evrdb.Database
	disable       bool
}

func (odr *testOdr) Database() evrdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db evrdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes2(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db evrdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db evrdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 0:
		...
		txValue := new(big.Int).Mul(big.NewInt(1100000), big.NewInt(params.GasPriceConfig))
		tx, _ := types.SignTx(
	...
	case 1:
		...
		nonce++
		tx3, _ := types.SignTx(
			types.NewContractCreation(nonce, big.NewInt(0), 1000000, big.NewInt(params.GasPriceConfig), testContractCode),
			signer,
			acc1Key,
		)
