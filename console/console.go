...
import (
	...
	"github.com/evrynet-official/evrynet-client/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Evrynet requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var evr = web3.eth; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines evr.getBalance(evr.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + evr.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + evr.blockNumber + " (" + new Date(1000 * evr.getBlock(evr.blockNumber).timestamp) + ")\n";
		try {
