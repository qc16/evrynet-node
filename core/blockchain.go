...
type BlockChain struct {
	...
	db     evrdb.Database // Low level persistent database to store final content in
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db evrdb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db evrdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - evrdb.IdealBatchSize)
			}
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, []interface{}, []*types.Log, error) {
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next() {
		...
		// Clique & Tendermint blocks where they can share state among each other, so importing an
		// older block might complete the state of the subsequent one. In this case,
		// just skip the block (we already validated it once fully (and crashed), since
		// its header and body was already in the database).
		if err == ErrKnownBlock {
			logger := log.Debug
			if bc.chainConfig.Clique == nil || bc.chainConfig.Tendermint == nil {
				logger = log.Warn
			}
			...
			// Clique & Tendermint blocks to have the same state is if there are no transactions.
