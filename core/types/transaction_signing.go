...
func Provider(signer Signer, tx *Transaction) (*common.Address, error) {
	...
	//Short circuit
	if (tx.data.PV != nil && tx.data.PV.Cmp(big.NewInt(0)) == 0) && (tx.data.PR != nil && tx.data.PR.Cmp(big.NewInt(0)) == 0) && (tx.data.PS != nil && tx.data.PS.Cmp(big.NewInt(0)) == 0) {
		return nil, nil
	}
