...
func testGetNodeData(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		...
		case 0:
			...
			txValue := new(big.Int).Mul(big.NewInt(100000), big.NewInt(params.GasPriceConfig))
			tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBank), acc1Addr, txValue, params.TxGas, big.NewInt(params.GasPriceConfig), nil), signer, testBankKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more ether to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBank), acc1Addr, big.NewInt(1000), params.TxGas, big.NewInt(params.GasPriceConfig), nil), signer, testBankKey)
			tx2, _ := types.SignTx(types.NewTransaction(block.TxNonce(acc1Addr), acc2Addr, big.NewInt(1000), params.TxGas, big.NewInt(params.GasPriceConfig), nil), signer, acc1Key)
			block.AddTx(tx1)
			block.AddTx(tx2)
		case 2:
...
func testGetReceipt(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		...
		case 0:
			...
			txValue := new(big.Int).Mul(big.NewInt(100000), big.NewInt(params.GasPriceConfig))
			tx, _ := types.SignTx(
				types.NewTransaction(block.TxNonce(testBank), acc1Addr, txValue, params.TxGas, big.NewInt(params.GasPriceConfig), nil),
				signer,
				testBankKey,
			)
			block.AddTx(tx)
		case 1:
			...
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignTx(
				types.NewTransaction(block.TxNonce(testBank), acc1Addr, big.NewInt(1000), params.TxGas, big.NewInt(params.GasPriceConfig), nil),
				signer,
				testBankKey,
			)
			tx2, _ := types.SignTx(
				types.NewTransaction(block.TxNonce(acc1Addr), acc2Addr, big.NewInt(1000), params.TxGas, big.NewInt(params.GasPriceConfig), nil),
				signer,
				acc1Key,
			)
			block.AddTx(tx1)
...
// Tests that post evr protocol handshake, clients perform a mutual checkpoint
