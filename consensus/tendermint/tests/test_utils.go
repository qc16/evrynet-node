...
//MakeNodeKey return random private node key
func MakeNodeKey() *ecdsa.PrivateKey {
	key, _ := GeneratePrivateKey()
	return key
}

//MustStartTestChainAndBackend init test chain & backend for test
func MustStartTestChainAndBackend(nodePK *ecdsa.PrivateKey, genesisHeader *types.Header) (*testChain, *backend.Backend) {
	...
	b, ok := backend.New(config, nodePK, backend.WithTxPoolOpts(&transaction.TxPoolOpts{CoreTxPool: pool}), backend.WithDB(memDB)).(*backend.Backend)
	if !ok {
...
func MakeGenesisHeader(validators []common.Address) *types.Header {
	...
	extra, _ := backend.PrepareExtra(header)
...
func MakeBlockWithSeal(engine *backend.Backend, pHeader *types.Header) *types.Block {
	header := makeHeaderFromParent(types.NewBlockWithHeader(pHeader))
	AppendSeal(header, engine)
	return types.NewBlockWithHeader(header)
}

func MustMakeBlockWithCommittedSealInvalid(engine *backend.Backend, pHeader *types.Header) *types.Block {
	header := makeHeaderFromParent(types.NewBlockWithHeader(pHeader))
	AppendSeal(header, engine)
...
func MustMakeBlockWithCommittedSeal(engine *backend.Backend, pHeader *types.Header, validators []common.Address) *types.Block {
	header := makeHeaderFromParent(types.NewBlockWithHeader(pHeader))
	AppendSeal(header, engine)
...
func AppendSeal(header *types.Header, engine *backend.Backend) {
	// sign the hash
