...
func (w *wizard) configStakingSC(genesis *core.Genesis, validators []common.Address) bool {
	fmt.Println()
	...
	if err != nil {
		log.Error("failed to compile SC file", "error", err)
		return false
	}
	...
	contractBackend, smlSCAddress := deployStakingSCToSimulatedBE(genesis, err, *abiSC, bytecodeSC, stakingSCParams)
	//Then get Code & Storage of SC to assign to new address
	codeOfSC, storageOfSC := getStakingSCData(err, contractBackend, smlSCAddress)
	...
	minValidatorStake := stakingSCParams[len(stakingSCParams)-3].(*big.Int)
	genesis.Alloc[*expectedSCAddress] = core.GenesisAccount{
...
func deployStakingSCToSimulatedBE(genesis *core.Genesis, err error, parsedABI abi.ABI, byteCodeSC string, stakingSCParams []interface{}) (*backends.SimulatedBackend, common.Address) {
	...
	return contractBackend, smlSCAddress
}

func getStakingSCData(err error, contractBackend *backends.SimulatedBackend, smlSCAddress common.Address) ([]byte, map[common.Hash]common.Hash) {
